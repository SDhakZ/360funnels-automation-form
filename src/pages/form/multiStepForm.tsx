...
// normalize validation paths like "bestSellingProducts[0]" or "address.city"
const topKey = (path = "") => path.split(/[.[]/).filter(Boolean)[0];
// Find first error by your on-screen order
const firstErroredKey = (
...
const scrollFocusTo = (name: string | undefined, headerOffset = 72) => {
  if (!name) return;
...
export default function MultiStepFormWithRedux() {
  ...
  const brandBookRef = React.useRef<Array<{ id: string; file: File }>>([]); // [{id,file}]
  const { step, step1, step2, step3 } = useSelector((s: RootState) => s.form);
  const submissionLoading = useSelector((s: RootState) => s.form.loading);
  ...
  const allowedStep = React.useMemo(() => {
    const isStep1Valid = step1ValidationSchema.safeParse(step1).success;
    const isStep2Valid = step2ValidationSchema.safeParse(step2).success;
    if (isStep1Valid && isStep2Valid) return 3;
    if (isStep1Valid) return 2;
    return 1;
  }, [step1, step2]);

  const validateStep = async (): Promise<{ ok: boolean; first?: string }> => {
    const schemas: Record<number, any> = {
      1: step1ValidationSchema,
      2: step2ValidationSchema,
      3: step3ValidationSchema,
    };
    const stepData: Record<number, unknown> = { 1: step1, 2: step2, 3: step3 };
    ...
    try {
      ...
      if (!result.success) {
        ...
        result.error.issues.forEach((issue: any) => {
          const path = issue.path?.length ? String(issue.path[0]) : "form";
