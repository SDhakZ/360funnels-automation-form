...
interface ColorPreferencesSectionProps {
...
export default function ColorPreferencesSection({
  ...
  errors = {},
  required = false,
}: ColorPreferencesSectionProps) {
  const handleChange = (
    e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>,
  ) => onChange(e.target.name, e.target.value);

  // Normalize on blur: add # if missing, uppercase; accept 3 or 6 hex digits
  const normalizeHexOnBlur = (name: string, raw: string) => {
    if (!raw) return; // allow empty (schema decides if required)
    ...
    if (v.length === 3 || v.length === 6) {
      ...
      // leave as-is; schema error will show
      onChange(name, raw);
    }
  };

  // For the color picker we must provide a valid hex or fallback
  const asPickerValue = (val: unknown, fallback = "#FFFFFF"): string => {
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-1">
        ...
        <Textarea
          ...
          rows={3}
          value={(data.otherColors as string) || ""}
